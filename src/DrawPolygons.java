...
public class DrawPolygons extends Component{
    ...
    public DrawPolygons(){
        polygonNames = new ArrayList<>(10);//maybe write all the names here instead of creating a hard coded list. Can always add more
    ...
    @Override
    public void paint(Graphics g) {//maybe just have a polygon method instead that takes in everything? so intstead of typing out everyhting we can minimize it.
        for (int i = 0; i < polygonNames.size(); i++) {
            ...
            if (currentPolygon.equals("square"))
                ...
                g.drawLine(currentCenter.x, currentCenter.y-10, currentCenter.x-10, currentCenter.y+10);//instead of current center write cC and make a method that takes in an offset instead.
